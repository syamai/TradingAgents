...
def record_submission(family_key: str, results: list[dict], *, db_path: str | None = None) -> int:
    ...
    반환: 제출 번호. 한도 초과·변형 수 초과는 ValueError.
    """
    if not results:
    ...
    with con:
        sub = con.execute(
            "SELECT submissions FROM families WHERE family_key=?", (family_key,)
        ).fetchone()[0] + 1
        for r in results:
            con.execute(
        ...
        new_status = ("passed" if passed
                      else ("exhausted" if sub >= MAX_SUBMISSIONS else "active"))
        con.execute(
            "UPDATE families SET submissions=?, status=? WHERE family_key=?",
            (sub, new_status, family_key),
        )
    return sub
