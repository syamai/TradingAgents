...
| **Hermes stock-analyst skill** | Korean stocks, swing horizon (2–8 weeks). Hermes calls trading-ai's MCP tools, applies 27 domain seeds, emits a hypotheses JSON, persists it, then collects user feedback + auto KOSPI-relative labels later. | Hermes itself = Codex Sonnet 4.6. Underlying *analysts* still run on local Ollama gemma (cost 0). | `tradingagents/hermes/mcp_server.py` (stdio), Hermes invokes it. |
